...
params.cappingType = Param(
    Categorical(("median", "mean", "worst", "best")), condition=Symbol("capping") == "1"
)
params.boundType = Param(
    Categorical(("candidate", "instance")), condition=Symbol("capping") == "1"
)
...
def default_instance(model):
    return (
        model,
        get_instances_training(model.instances),
        get_instances_validation(model.instances),
    )
